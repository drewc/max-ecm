...
(defclass claim-transaction (record)
  ((id :initarg :transaction-id)
   ...
   (approved :initarg :approved
	     :accessor claim-transaction-approved)))
...
(defmethod object-jso ((object claim-transaction) &key &allow-other-keys)
  (max-ecm/json:jso 
   ...
   "payee" (let ((id (claim-transaction-payee-id object)))
		 ...
		 id))
   "approved" (claim-transaction-approved object)))
...
(defun from-bool (thing)
  (cond ((string= thing "%:false:%")
	 nil)
	((string= thing "on")
	 t)	
	(t (from-json-bool thing))))
...
(defmethod update-record/jso  ((record claim-transaction) jso)
  (let ((cheque-number (getjso "cheque_number" jso)))
	...
	(getjso "payee" jso)
      (multiple-value-bind (approved approved-exists?)
	  (getjso "approved" jso)
	...
	(postmodern:execute 
	 (s-sql:sql-compile 
	  `(:update claim-transaction 
	    ...
	    ,@(when approved-exists? (list 'approved (from-bool approved)))
